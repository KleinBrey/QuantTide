...
from ..database import USDuckDBDatabase
from ..utils.symbol import normalize_daily_bar_symbol

US_STOCK_COLUMNS = ["symbol", "name", "source"]

US_DAILY_BAR_COLUMNS = [
    "symbol",
    "trade_date",
    "open",
    "high",
...
class BaseRepository:
    """保存美股 Repository 共用的数据库依赖。"""

    def __init__(self, db: USDuckDBDatabase):
        self.db = db


class USStockRepository(BaseRepository):
    """负责美股库 ``stocks`` 表的读写。"""
    ...
    def upsert_stocks(self, rows: pd.DataFrame) -> int:
        ...
        _require_columns(stocks, US_STOCK_COLUMNS)
        stocks = stocks[US_STOCK_COLUMNS].copy()

        for column in US_STOCK_COLUMNS:
        ...
        with self.db.connection() as connection:
            connection.register("incoming_us_stocks", stocks)
            connection.execute("""
                INSERT INTO stocks (symbol, name, source)
                SELECT symbol, name, source
                FROM incoming_us_stocks
    ...
    def delete_stocks(self, symbols: str | Iterable[str]) -> int:
        ...
        with self.db.connection() as connection:
            connection.register("deleting_us_stocks", deleting)
            affected_rows = connection.execute("""
                SELECT count(*)
                FROM stocks
                JOIN deleting_us_stocks USING (symbol)
                """).fetchone()[0]
            connection.execute("""
                DELETE FROM stocks
                USING deleting_us_stocks
                WHERE stocks.symbol = deleting_us_stocks.symbol
                """)
        return affected_rows


class USDailyBarRepository(BaseRepository):
    """负责美股库 ``daily_bars`` 表的读写。"""
    ...
    def get_by_symbol_and_date_range(
        ...
        normalized_symbol = normalize_daily_bar_symbol(symbol, "us-share")
    ...
    def upsert_daily_bars(self, rows: pd.DataFrame) -> int:
        ...
        if "amount" not in bars.columns:
            bars["amount"] = None
        _require_columns(bars, US_DAILY_BAR_COLUMNS)
        bars = bars[US_DAILY_BAR_COLUMNS]
        bars["trade_date"] = pd.to_datetime(bars["trade_date"], errors="raise").dt.date

        with self.db.connection() as connection:
            connection.register("incoming_us_daily_bars", bars)
            connection.execute("""
...
class USStockHotDailyRepository(BaseRepository):
    """负责美股库 ``stock_hot_daily`` 表的读写。"""
    ...
    def upsert_stock_hot_daily(self, rows: pd.DataFrame) -> int:
        ...
        _require_columns(hot_rows, US_STOCK_HOT_DAILY_COLUMNS)
        hot_rows = hot_rows[US_STOCK_HOT_DAILY_COLUMNS]
